...
BunkerBaseRos::BunkerBaseRos(std::string node_name)
  ...
  this->declare_parameter("command_timeout", rclcpp::ParameterValue(0.25));
...
void BunkerBaseRos::LoadParameters() {
  ...
  this->get_parameter_or<double>("command_timeout", command_timeout_, 0.25);
  ...
  std::cout << "- publish odom tf: " << std::boolalpha << publish_odom_tf_
            << std::endl;
  std::cout << "- command timeout: " << command_timeout_ << " s" << std::endl;
...
void BunkerBaseRos::Run() {
  ...
  messenger->SetCommandTimeout(command_timeout_);
