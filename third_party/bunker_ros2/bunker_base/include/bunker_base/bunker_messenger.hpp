...
class BunkerMessenger {
 public:
  ...
  void SetCommandTimeout(double timeout) { command_timeout_ = timeout; }
  ...
  void PublishStateToROS() {
    ...
    if (command_received_ && command_timeout_ > 0.0 &&
        (current_time_ - last_command_time_).seconds() > command_timeout_) {
      bunker_->SetMotionCommand(0.0, 0.0);
